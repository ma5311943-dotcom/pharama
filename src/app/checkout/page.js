    ...
    if (paymentMethod === 'Credit Card') {
      ...
      try {
        ...
        if (!orderJson.success) {
          toast.error(orderJson.error || 'Failed to create order in database');
        ...
        const res = await fetch('/api/checkout', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ items, email: user?.email, orderId: orderJson.data._id })
        });
        const data = await res.json();
        if (data.url) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-6">
            <motion.div
              ...
              className="bg-bg-card p-6 rounded-[2rem] border border-border-nav shadow-soft flex items-center justify-between"
            >
              <div>
            ...
            <motion.div
              ...
              className="bg-bg-card p-6 sm:p-8 rounded-[2.5rem] border border-border-nav shadow-soft space-y-6"
            >
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { name: 'Credit Card', color: 'bg-blue-50 text-blue-600 border-blue-200' },
              ...
              <div className="bg-bg-page p-6 rounded-2xl border border-border-nav">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    { step: '02', text: paymentMethod === 'Credit Card' ? 'Enter Card Details on Stripe' : 'Screenshot the receipt' },
                    { step: '03', text: paymentMethod === 'Credit Card' ? 'Automatic Order Confirmation' : 'Upload it below' },
                    { step: '04', text: 'Wait for delivery' },
                  ].map(s => (
                    <div key={s.step} className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-lg bg-bg-card border border-border-nav flex items-center justify-center text-[10px] font-bold text-primary shrink-0 shadow-sm">
                        {s.step}
                      </div>
                      <p className={cn(
                      ...
                      {screenshotPreview ? (
                        <div className="relative w-full h-full p-2">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center rounded-xl">
                            <div className="bg-bg-card p-3 rounded-full shadow-lg">
          ...
          <div className="lg:col-span-2 space-y-6">
            <motion.div
              ...
              className="bg-bg-card rounded-[2.5rem] p-6 border border-border-nav shadow-soft sticky top-24"
            >
              ...
              <div className="space-y-3.5 pt-6 border-t border-border-nav">
